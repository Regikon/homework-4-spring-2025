...
class TestCreateLeadFormQuestionPage(BaseCase):
    ...
    def test_create_question(self, question_page):
        ...
        assert question_page.has_element(self.locators.QUESTION_EXISTS)
        question_page.enter_question_text(self.QUESTION_TEXT)
        assert question_page.has_element(
            self.locators.HAS_QUESTION_TEXT_ON_PREVIEW(self.QUESTION_TEXT)
        )
        question_page.add_answer_text(1, self.ANSWER_1_TEXT)
        question_page.add_answer_text(2, self.ANSWER_2_TEXT)
        assert question_page.has_element(
            self.locators.HAS_ANSWER_ON_PREVIEW(self.ANSWER_1_TEXT)
        )
        assert question_page.has_element(
            self.locators.HAS_ANSWER_ON_PREVIEW(self.ANSWER_2_TEXT)
        )
    ...
    def test_delete_question(self, question_page):
        ...
        assert not question_page.has_element(self.locators.QUESTION_EXISTS)

    def test_question_switch_type(self, question_page):
        question_page.add_question()
        assert question_page.has_element(self.locators.QUESTION_EXISTS)
        question_page.switch_question_type_many_answers()
        assert question_page.has_element(self.locators.HAS_QUESTION_TYPE_MANY_ANSWERS_ON_PREVIEW)
    ...
    def test_add_new_answer_in_question(self, question_page):
        ...
        assert question_page.has_element(
            self.locators.HAS_ANSWER_ON_PREVIEW(self.ANSWER_3_TEXT)
        )

    def test_add_contact_field_city(self, question_page):
        question_page.add_contact_field(self.CONTACT_NEW)
        assert question_page.has_element(self.locators.HAS_CONTACTS_NAME_ON_PREVIEW)
        assert question_page.has_element(self.locators.HAS_CONTACTS_TELEPHONE_ON_PREVIEW)
        assert question_page.has_element(self.locators.HAS_CONTACTS_CITY_ON_PREVIEW)
    
    def test_go_to_result_page(self, question_page):
        question_page.go_to_result_page()
        assert question_page.has_element(self.locators.RESULT_PAGE_ACTIVE)
